      ...
      {!isEmpty && isLatestMonth ? (
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center gap-6">
            {/* 順位 */}
            <span className={`font-bold text-xl ${getRankColor(entry.rank)}`}>{entry.rank}位</span>

            {/* 的の大きさ */}
            <span className="text-xl font-bold text-accent">{entry.targetSize}</span>
          ...
          <div className="flex items-center gap-6 text-sm text-gray-600 ml-2">
            {/* 更新日 */}
            <span>(更新日){formatShortDate(entry.updatedDate)}</span>

            {/* 有効期限 */}
            <span>(有効期限){formatShortDate(entry.expiryDate)}</span>
          </div>
        </div>
      ) : (
        /* 最新月以外、または空の場合は1行表示 */
        <div className="flex items-center gap-6">
          {/* 順位 */}
          <span
            className={`font-bold text-xl ${isEmpty ? "text-gray-400" : getRankColor(entry.rank)}`}
          >
            {entry.rank}位
          </span>
