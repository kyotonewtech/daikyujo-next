...
function main() {
  ...
  for (const [personId] of personMap) {
    if (personId.startsWith('person_')) {
      const idNum = parseInt(personId.replace('person_', ''));
      if (!isNaN(idNum) && idNum > maxPersonId) {
        maxPersonId = idNum;
      }
    }
  }
  ...
  const registry: PersonRegistry = {
    ...
    persons: Array.from(personMap.values()).sort((a, b) => {
      const aNum = parseInt(a.personId.replace('person_', ''));
      const bNum = parseInt(b.personId.replace('person_', ''));
      return aNum - bNum;
    })
  };
